    ...
    parseAuthenticationResponse(offset, length, bytes) {
        ...
        switch (code) {
            ...
            case 5: // AuthenticationMD5Password
                ...
                break;
            // AuthenticationSASL
            // !需注意: OpenGauss 和 PostgreSQL 在处理 sha256 认证鉴权时行为不一样
            // 这里参考 RFC5802 文档实现: https://datatracker.ietf.org/doc/html/rfc5802
            case 10:
                // 消息格式:
                // 4字节 {ascii string} method 
                // 64字节 {ascii string} random64code
                // 8字节 {ascii string} token
                // 4字节 {int32} serverIteration
                message.name = "authenticationSASL" /* authenticationSASL */;
                message.method = this.reader.bytes(4).readUInt32BE(0);
                message.random64code = this.reader.bytes(64).toString('ascii');
                message.token = this.reader.bytes(8).toString('ascii');
                message.serverIteration = 10000;
                break;
